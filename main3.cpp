...
public:
    ...
    void reserveRoom() {
		    ...
		    if (roomNumber >= 1 && roomNumber <= 15 && !rooms[roomNumber - 1].roomName.empty()) {
		        ...
		        cout << "Room " << rooms[roomNumber - 1].roomName << " reserved. Reference number: " << reservation.referenceNumber << endl;
	    ...
	    // Find the reservation with the given reference number
	    auto it = reservations.begin();
	    while (it != reservations.end()) {
	        if (it->referenceNumber == referenceNumber) {
	            it = reservations.erase(it);  // Remove the reservation from the vector
	            cout << "Reservation with reference number " << referenceNumber << " deleted." << endl;
	            updateReservationFile(); // Update the reservations CSV file
	            return;
	        } else {
	            ++it;
	        }
    ...
    void loadReservations() {
	    ...
	    if (file) {
	        ...
	        for (const auto& reservation : reservations) {
	            if (reservation.referenceNumber > highestReferenceNumber) {
	                highestReferenceNumber = reservation.referenceNumber;
	            }
	        }
        ...
        if (reservations.empty())
		{
    	...
    	} else 
			{
		        for (const auto& reservation : reservations)
				 {
		            cout << "+-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" << endl;
		            cout << "| Reference number: " << reservation.referenceNumber << endl;
